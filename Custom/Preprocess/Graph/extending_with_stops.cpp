...
RTree index_graph_nodes(vector<Edge> const& edges) {
    RTree rtree;
    for (auto edge : edges) {
...
Node get_closest_node(RTree const& rtree, Stop const& stop) {
    ...
    Node closest_node = closest_nodes.front().second;
    return closest_node;
}
...
pair<vector<Edge>, vector<StopWithClosestNode>> extend_graph(vector<Stop> const& stops, vector<Edge> const& edges, float walkspeed_km_per_h) {
    ...
    vector<StopWithClosestNode> stops_with_closest_node;
    for (auto& stop : stops) {

        Node closest_node = get_closest_node(rtree, stop);

        // we now extend graph with a straight edge from stop to closest node :
        Polyline geometry{{stop.lon, stop.lat}, {closest_node.lon(), closest_node.lat()}};
