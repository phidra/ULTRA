...
void dump_geojson_graph(ostream& out, vector<Edge> const& edges, bool allow_unranked) {
    ...
    for (auto& edge : edges) {
        ...
        /* properties.AddMember("node_from_rank", allow_unranked ? edge.node_from.get_rank_or_unranked() :
         * edge.node_from.get_rank(), a); */
        properties.AddMember("node_from", rapidjson::Value().SetString(edge.node_from.id.c_str(), a), a);
        /* properties.AddMember("node_to_rank",  allow_unranked ? edge.node_to.get_rank_or_unranked() :
         * edge.node_to.get_rank(), a); */
